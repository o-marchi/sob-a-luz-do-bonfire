...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`campaigns_players_order_idx\` ON \`campaigns_players\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`campaigns_players_parent_id_idx\` ON \`campaigns_players\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`campaigns_players_player_idx\` ON \`campaigns_players\` (\`player_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_campaigns_id_idx\` ON \`payload_locked_documents_rels\` (\`campaigns_id\`);`)
