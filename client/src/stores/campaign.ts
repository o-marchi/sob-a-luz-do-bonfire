...
import { ref } from 'vue'
import type { Campaign, CampaignUser } from '@/types/Campaign.ts'
...
export const useCampaignStore = defineStore('campaign', () => {
  ...
  const campaignUser = ref<CampaignUser | null>(null)
  ...
  async function init() {
    ...
    if (user) {
      campaignUser.value =
        campaign.value?.users?.find((campaignUser: CampaignUser) => campaignUser.id === user?.id) ?? null
    } else {
